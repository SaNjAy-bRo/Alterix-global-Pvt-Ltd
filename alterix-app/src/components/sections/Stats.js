...
export default function StatsSection() {
  return (
    <section className="relative bg-[#0B1120] py-32 overflow-hidden z-10 [clip-path:polygon(0_0,100%_4vw,100%_100%,0_100%)] mt-[-4vw]">
      ...
      <div className="absolute top-1/2 left-0 w-full h-[600px] bg-[#EA580C] rounded-full blur-[200px] opacity-[0.02] pointer-events-none -translate-y-1/2"></div>
      ...
      <div className="container mx-auto px-8 relative z-10 pt-8">
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          ...
          <div className="flex-1 text-white">
            <span className="text-[#EA580C] font-extrabold tracking-[0.2em] uppercase mb-4 block flex items-center gap-2">
              <Globe2 size={18} /> Global Footprint
            </span>
            <h2 className="text-5xl md:text-6xl font-black uppercase mb-6 leading-tight">
              Scale without <br className="hidden md:block"/><span className="text-transparent bg-clip-text bg-gradient-to-r from-[#EA580C] to-white">Compromise.</span>
            </h2>
            <p className="text-slate-400 text-lg md:text-xl leading-relaxed mb-8 border-l-4 border-[#EA580C] pl-6 max-w-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-10 text-sm font-bold tracking-widest uppercase text-slate-300">
              <div className="flex items-center gap-3 bg-white/5 px-4 py-3 rounded-sm border border-white/10 hover:border-[#EA580C]/50 transition-colors">
                <Cpu className="text-[#EA580C]" size={20} /> Localized Engineering
              </div>
              <div className="flex items-center gap-3 bg-white/5 px-4 py-3 rounded-sm border border-white/10 hover:border-[#EA580C]/50 transition-colors">
                <Activity className="text-[#EA580C]" size={20} /> Real-Time Analytics
              </div>
            </div>

            <Button className="bg-[#EA580C] hover:bg-white hover:text-[#EA580C] text-white px-8 py-7 md:py-8 text-base uppercase tracking-widest font-black transition-all duration-300 shadow-xl group rounded-sm border-none">
              ...
              {statData.map((stat) => {
                ...
                return (
                  <Card key={stat.id} className="relative bg-white/5 backdrop-blur-md border-white/10 shadow-2xl hover:-translate-y-2 transition-all duration-500 rounded-lg group overflow-hidden z-10 cursor-default flex flex-col h-full">
                    ...
                    {/* Hover Glow Effect */}
                    <div className="absolute -inset-1 bg-gradient-to-r from-[#EA580C] to-transparent opacity-0 group-hover:opacity-20 transition-opacity duration-700 blur-xl"></div>
                    
                    <CardContent className="p-8 md:p-10 text-left flex flex-col h-full relative z-10">
                      <div className="w-14 h-14 rounded-md flex items-center justify-center text-[#EA580C] bg-[#EA580C]/10 group-hover:bg-[#EA580C] group-hover:text-white transition-all duration-500 mb-6 shadow-inner ring-1 ring-[#EA580C]/30">
                      ...
                      <div className="text-lg font-bold text-[#EA580C] uppercase tracking-wider mb-4 border-b border-white/10 pb-4">
                        {stat.label}
                      </div>
